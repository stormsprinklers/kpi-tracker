...
async function logWebhookInRoute(organizationId: string, rawBody: string, request: Request) {
  console.log("[WH-DBG] HCP-ROUTE logWebhookInRoute", JSON.stringify({ organizationId, rawBodyLen: rawBody?.length }));
  try {
    await initSchema();
    ...
    console.log("[WH-DBG] HCP-ROUTE insertWebhookLog succeeded", JSON.stringify({ organizationId }));
  } catch (err) {
    console.error("[WH-DBG] HCP-ROUTE insertWebhookLog FAILED", err);
  }
}
