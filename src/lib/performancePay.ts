...
export interface PayrollExportDetail {
  /** Straight-time bucket: up to 40h per Mon–Sun workweek (hours in the selected date range only). */
  regularHours: number;
  /** Hours beyond 40 in each Mon–Sun workweek (same rule as hourly OT pay math). */
...
/** Regular / OT split by Mon–Sun workweek (FLSA-style), using timesheet hours in the selected range per week. */
function regularOvertimeHoursFromWeeklyBuckets(
  hoursByWeek: Map<string, number>
): { regularHours: number; overtimeHours: number } {
  let regular = 0;
  let overtime = 0;
  for (const raw of hoursByWeek.values()) {
    const weekHours = Math.round(raw * 100) / 100;
...
function buildPayrollGuide(
  ...
  switch (st) {
    case "pure_hourly":
      ...
      lines.push(
        "Regular vs. OT: up to 40h per Mon–Sun workweek (timesheet hours in this date range only)."
      );
      break;
    case "hourly_commission_tiers":
      lines.push(
      ...
      lines.push(
        "Regular vs. OT: 40h per Mon–Sun workweek in this date range; the hourly leg does not apply a 1.5× OT multiplier in the app."
      );
      break;
    case "hourly_to_commission": {
    ...
    case "hourly_metrics":
      ...
      lines.push(
        "Regular vs. OT in this table use 40h per Mon–Sun workweek in the selected range; pay uses total hours × tier rate (no OT premium in app)."
      );
      break;
    case "csr_hourly_booking_rate":
      lines.push(
...
export async function calculateExpectedPay(
  ...
  /** empId -> (week Monday YMD -> hours in that week) — used for hourly OT pay math and reg/OT display. */
  ...
  for (const e of timeEntries) {
    ...
    byWeek.set(weekKey, Math.round(((byWeek.get(weekKey) ?? 0) + h) * 100) / 100);
  }
